...
| **빈 땅 드래그** (객체 선택 상태) | 선택한 객체를 드래그 지점으로 즉시 이동 (자유 카메라 제외) |
...
| `5` | Scout First-Person (선택된 운용자 1인칭) |
...
- 처음 열면 **간단 모드**: 필수 컨트롤(실행·일시정지·리셋·배속·분석)만 보입니다.
  하단 **고급 설정 ▸** 버튼으로 전체 UI(편집·리플레이·스텝·카메라·오버라이드·
  시나리오 저장/불러오기·이벤트 로그)를 켤 수 있습니다.
- 하단 **분석** 버튼: 추정 오차(실선)와 불확실성(점선) 추이 · 스카우트별 RSSI ·
  실제 vs 추정 궤적(XY) 차트와 **CSV 내보내기**(`sim_log.csv`)를 제공합니다.
  데이터는 실행 중 0.2 s 간격으로 자동 기록되는 리플레이 버퍼를 그대로 사용합니다.
- Scout를 선택하면 우측 패널에 **RF 스펙트럼 (모의)** 워터폴이 표시됩니다.
  진폭은 해당 Scout의 실제 RSSI에 연동된 합성 신호(FHSS 호핑 + 비디오 밴드)로,
  추후 IQ→STFT 백엔드로 교체 가능한 자리표시자입니다.
...
## 7. RF 모델 — 물리 전파 + 융합 휴리스틱

Scout별 RSSI/SNR은 **링크버짓 기반 물리 모델**([src/sim/propagation.ts](src/sim/propagation.ts))로 계산합니다:

1. Friis 자유공간손실 + 로그거리 경로손실(n=2.2) + 로그정규 섀도잉(σ=5dB)
2. `RSSI = Ptx + Gtx + Grx − PL`, 열잡음 바닥 `N = −174dBm/Hz + 10·log10(B) + NF`
3. `SNR = RSSI − N`, SNR 14dB(±1.5dB 히스테리시스) + 신뢰도 램프에서 탐지 확정
   → 탐지 반경이 물리에서 **창발**: 2.45GHz ≈ **1.33km**, 5.8GHz ≈ **0.61km**
   (우측 패널에서 드론 주파수 대역 선택 가능)
4. RSSI를 물리식 역산으로 거리 추정 → 탐지 Scout 수에 따라 3+ → **가중
   최소제곱 다변측량**, 2 → 원 교점(이전 추정에 가까운 해), 1 → 거리 링 위의 점
5. 불확실성 = 거리 잔차 + 기하 패널티(Scout 수) + 신뢰도 항

수식 근거·상수 출처·검증 방법은 [docs/propagation-model.md](docs/propagation-model.md),
물리 검증은 `npm run verify:propagation`(16체크) / `npm run verify:rf`(10체크)로
실행할 수 있습니다.
